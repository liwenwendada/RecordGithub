...
function PauseLayer:init()
	...
	local function click(event)
		...
		if tag == 1 then--重玩
			...
			cc.Director:getInstance():replaceScene(require("app.scenes.GameScene").new())
		elseif tag == 2 then --继续
			cc.Director:getInstance():resume()
			self:removeFromParent()
		elseif tag == 3 then --回到关卡选择界面
			cc.Director:getInstance():resume()
			cc.Director:getInstance():replaceScene(require("app.scenes.SelectChapter").new())
		end
	end
