...
function StartScene:init()
	...
	self._sp:pos(display.right-150, display.top-150)
	self._sp:setScale(2)
	self:addChild(self._sp,1)
	self:startAnimate()

	self._snow = cc.ParticleSnow:create()
	self._snow:pos(display.cx, display.top)
	self:addChild(self._snow,1,1)


     local BubbleButton = require("app.class.BubbleButton")
    --开始按钮
    ...
    local selectScene = require("app.scenes.SelectScene").new()
   ...
   --                   :addTo(self)
    ...
    self.menu:pos(display.cx, display.cy)
